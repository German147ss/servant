...
class Transactions extends CI_Controller{
    ...
    public function __construct(){
        ...
        $this->load->model(['transaction', 'item' , 'employee']);
    }
    ...
    */
    ...
    public function index(){
        $transData['items'] = $this->item->getActiveItems('name', 'ASC');//get items with at least one qty left, to be used when doing a Nueva Venta
    ...
    public function nso_(){
        ...
        if($allIsWell){//insert each sales order into db, generate receipt and return info to client
            ...
            $returnedData = $this->insertTrToDb($arrOfItemsDetails, $_mop, $_at, $cumAmount, $_cd, $this->vat_amount, $vatPercentage, $this->discount_amount,
                    $discount_percentage, $cust_code, $cust_name, $cust_phone, $cust_email, $localO);
            $dinero = $cumAmount;
            $this->employee->addPoints($cust_code,$dinero);
    ...
    */
    ...
    private function validateItemsDet($arrOfItemsInfo, $cumAmountFromClient, $amountTendered, $vatPercentage, $discount_percentage){
        ...
        foreach ($arrOfItemsInfo as $get){
          ...
          //use this to get the item's unit price, then multiply it with the qty sent from client
            $qtyToBuy = $get->qty;
    ...
    */
     ...
     * @return boolean
     */
    private function insertTrToDb($arrOfItemsDetails, $_mop, $_at, $cumAmount, $_cd, $vatAmount, $vatPercentage, $discount_amount, $discount_percentage, $cust_code, $cust_name, $cust_phone, $cust_email,$local){
        ...
        foreach($arrOfItemsDetails as $get){
            ...
            $transId = $this->transaction->add($itemName, $itemCode, "", $qtySold, $unitPrice, $totalPrice, $cumAmount, $_at, $_cd,
                    $_mop, 1, $ref, $vatAmount, $vatPercentage, $discount_amount, $discount_percentage, $cust_code, $cust_name, $cust_phone,
                    $cust_email,$local);
        ...
        $dataToReturn['transReceipt'] = $this->genTransReceipt($allTransInfo, $cumAmount, $_at, $_cd, $ref, $dateInDb, $_mop, $vatAmount, $vatPercentage, $discount_amount, $discount_percentage, $cust_name, $cust_phone, $cust_email,$local);
    ...
    */
    ...
    private function genTransReceipt($allTransInfo, $cumAmount, $_at, $_cd, $ref, $transDate, $_mop, $vatAmount, $vatPercentage,
        $discount_amount, $discount_percentage, $cust_name, $cust_phone, $cust_email, $cust_local){
    ...
    public function vtr_(){
        ...
        if($transInfo){
            ...
            $json['transReceipt'] = $this->genTransReceipt($transInfo, $cumAmount, $amountTendered, $changeDue, $ref,
                ...
                $cust_phone, $cust_email, $cust_local);
        }
    ...
    */
    ...
    public function reportItems($from_date, $to_date=''){
        //get all transactions from db ranging from $from_date to $to_date
        $datos['from'] = $from_date;
        $datos['to'] = $to_date ? $to_date : date('Y-m-d');

        $datos['allTransactionsItems'] = $this->transaction->getDateRangeItem($from_date, $to_date);

        $this->load->view('transactions/transReportItem', $datos);
    }
    ...
    public function delete(){
        ...
        if($item_id){
            $this->db->where('ref', $item_id)->delete('transactions');
