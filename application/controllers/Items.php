...
class Items extends CI_Controller{
    ...
    */
    ...
    public function lilt(){
        ...
        $data['cum_cost'] = $this->item->getItemsCumCost();
    ...
    */
    public function liltlocal(){
        ...
        $orderBy = $this->input->get('orderBy', TRUE) ? $this->input->get('orderBy', TRUE) : "name";
        $orderFormat = $this->input->get('orderFormat', TRUE) ? $this->input->get('orderFormat', TRUE) : "ASC";
        $local = $this->input->get('local', TRUE) ? $this->input->get('local', TRUE) : "local";
        ...
        //call setPaginationConfig($totalRows, $urlToCall, $limit, $attributes) in genlib to configure pagination
        $config = $this->genlib->setPaginationConfig($totalItems, "items/liltlocal", $limit, ['onclick'=>'return liltlocal(this.href);']);

        $this->pagination->initialize($config);//initialize the library class

        //get all items from db
        $data['allItems'] = $this->item->getAllLocal($orderBy, $orderFormat, $start, $limit, $local);
        $data['range'] = $totalItems > 0 ? "Showing " . ($start+1) . "-" . ($start + count($data['allItems'])) . " of " . $totalItems : "";
        $data['links'] = $this->pagination->create_links();//page links
        $data['sn'] = $start+1;
        $data['cum_total'] = $this->item->getItemsCumTotal();
    ...
    public function add(){
        ...
        if($this->form_validation->run() !== FALSE){
             ...
             */
            $insertedId = $this->item->add(set_value('itemName'), set_value('itemQuantity'), set_value('itemCost'), set_value('itemPrice'), set_value('itemLocal'),
            ...
            $json = $this->db->trans_status() !== FALSE ?
                    ['status'=>1, 'msg'=>"Producto añadido correctamente"]
                    :
    ...
    */
    ...
    public function gcoandqty(){
        ...
        if ($this->session->admin_local == "") {
          ...
          $arraynew = explode("-", $itemCode);
          $itemCode = $arraynew[0];
          $local = $arraynew[1];
        }else{
        $itemCode = $this->input->get('_iC', TRUE);
        $local = $this->input->get('local', TRUE);}
        if($itemCode){
            $item_info = $this->item->getItemInfo(['code'=>$itemCode], ['local'=>$local], ['quantity', 'unitPrice', 'description']);
    ...
    */
    ...
    public function edit(){
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $itemLocal = $this->input->post('itemLocal', TRUE);
            $itemPrice = set_value('itemPrice');
            $itemName = set_value('itemName');
            $itemCode = $this->input->post('itemCode', TRUE);
                
            //update item in db
            $updated = $this->item->edit($itemId, $itemName, $itemDesc, $itemCost, $itemPrice, $itemLocal);

            $json['status'] = $updated ? 1 : 0;
            $json['locals'] = $itemLocal;
            //add event to log
            //function header: addevent($event, $eventRowId, $eventDesc, $eventTable, $staffId)
