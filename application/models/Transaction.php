...
class Transaction extends CI_Model {
    ...
    public function getAll($orderBy, $orderFormat, $start, $limit) {
        if ($this->db->platform() == "sqlite3") {
            $q = "SELECT transactions.ref, transactions.totalMoneySpent, transactions.modeOfPayment, transactions.staffId,
                transactions.transDate, transactions.lastUpdated, transactions.amountTendered, transactions.local, transactions.changeDue,
            ...
            if($this->session->admin_local == "" || $this->session->admin_local == " " || is_null($this->session->admin_local) || empty($this->session->admin_local) ){

           }else{
             $this->db->where('transactions.local =', $this->session->admin_local);
           }
    ...
    public function getAllItems($orderBy, $orderFormat, $start, $limit) {

            $this->db->select('transactions.ref, transactions.itemName, transactions.itemCode, transactions.quantity, transactions.unitPrice, transactions.totalMoneySpent, transactions.amountTendered, transactions.totalPrice, transactions.local, transactions.transDate, unitCost');
            $this->db->join('items', 'transactions.itemCode = items.code');
            $this->db->limit($limit, $start);
            $this->db->group_by('ref');
            $this->db->order_by($orderBy, $orderFormat);
            $run_q = $this->db->get('transactions');
     ...
     * @param {string} $cc Customer Code
     * @param {string} $cn Customer Name
     * @param {string} $cp Customer Phone
     * @param {string} $ce Customer Email
     * @return boolean
     */
   public function add($_iN, $_iC, $desc, $q, $_up, $_tp, $_tas, $_at, $_cd, $_mop, $_tt, $ref, $_va, $_vp, $da, $dp, $cc, $cn, $cp, $ce, $_cl) {


     /*if($this->session->admin_local == "" || $this->session->admin_local == " "){
       $localsinho = $_cl;
     }else{
       $localsinho = $this->session->admin_local;
     }*/
        $data = ['itemName' => $_iN, 'itemCode' => $_iC, 'description' => $desc, 'quantity' => $q, 'unitPrice' => $_up, 'totalPrice' => $_tp,
            ...
            'vatPercentage' => $_vp, 'discount_amount'=>$da, 'discount_percentage'=>$dp, 'cust_code'=>$cc, 'cust_name'=>$cn, 'cust_phone'=>$cp,
            'cust_email'=>$ce, 'local'=> $_cl ];
  ...
  if($this->session->admin_local == "" || $this->session->admin_local == " " || is_null($this->session->admin_local) || empty($this->session->admin_local) ){
      ...
      public function getDateRangeItem($from_date, $to_date){

            $this->db->select('transactions.transId,transactions.ref,transactions.itemName, transactions.quantity, transactions.itemCode, transactions.totalPrice, transactions.unitPrice, transactions.local, transactions.totalMoneySpent, transactions.modeOfPayment, transactions.staffId, transactions.transDate, transactions.lastUpdated, transactions.amountTendered, transactions.changeDue, items.unitCost');

            $this->db->where("DATE(transDate) >= ", $from_date);
            $this->db->where("DATE(transDate) <= ", $to_date);
