...
$(document).ready(function(){
    ...
    $("#addNewItem").click(function(e){
        ...
        var itemDescription = $("#itemDescription").val();
                var itemLocal = $("#localForm").val();
                var itemCost= $("#itemCost").val();

        if(!itemName || !itemQuantity || !itemPrice || !itemCost || !itemCode){
            ...
            $("#addCustErrMsg").text("Uno o más campos obligatorios están vacíos");
            
            return;
        }
        
        displayFlashMsg("Agregando Producto '"+itemName+"'", "fa fa-spinner faa-spin animated", '', '');
        
        $.ajax({
            type: "post",
            url: appRoot+"items/add",
            data:{itemName:itemName, itemQuantity:itemQuantity, itemCost:itemCost, itemPrice:itemPrice, itemLocal:itemLocal, itemDescription:itemDescription, 			itemCode:itemCode},
            ...
            success: function(returnedData){
                if(returnedData.status === 1){
                    ...
                    $("#itemCostErr").text(returnedData.itemPrice);
    ...
    $("#itemsListPerPage, #itemsListSortBy").change(function(){
        displayFlashMsg("Espere por favor...", spinnerClass, "", "");
        lilt();
    });
    $("#itemsListSortByLocal").change(function(){
        displayFlashMsg("Espere por favor...", spinnerClass, "", "");
        liltlocal();
    });
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    $("#editItemSubmit").click(function(){
        ...
        var itemLocal = $("#itemLocalEdit").val();
        if(!itemName || !itemPrice || !itemId){
            !itemName ? $("#itemNameEditErr").html("Producto no puede estar vacio") : "";
            !itemPrice ? $("#itemPriceEditErr").html("Precio del producto no puede estar vacio") : "";
            !itemId ? $("#editItemFMsg").html("Unknown item") : "";
            return;
        }
        
        $("#editItemFMsg").css('color', 'black').html("<i class='"+spinnerClass+"'></i> Procesando su peticion....");
        
        $.ajax({
            method: "POST",
            url: appRoot+"items/edit",
            data: {itemName:itemName, itemCost:itemCost, itemPrice:itemPrice, itemDesc:itemDesc, _iId:itemId, itemCode:itemCode, itemLocal:itemLocal}
        }).done(function(returnedData){
            if(returnedData.status === 1){
                $("#editItemFMsg").css('color', 'green').html("Producto actualizado Correctamente");
                ...
                $("#editItemFMsg").css('color', 'red').html("Uno o mas Campos son requeridos o estan llenamos incorrectamente");
    ...
    $("#stockUpdateType").on('change', function(){
        ...
        if(updateType && (updateType === 'newStock')){
            $("#stockUpdateDescription").val("Recibimos nuevo cantidad.");
        }
    ...
    $("#stockUpdateSubmit").click(function(){
        ...
        $("#stockUpdateFMsg").html("<i class='"+spinnerClass+"'></i> Actualizando.....");
    ...
    $("#itemsListTable").on('click', '.delItem', function(e){
        ...
        if(itemId){
            var confirm = window.confirm("Esta seguro de querer borrar el producto? Esto no se puede deshacer.");
            ...
            if(confirm){
                ...
                $.ajax({
                    ...
                    if(rd.status === 1){
                        ...
                        changeFlashMsgContent('Producto Borrado', '', 'green', 1000);
                    }
...
function liltlocal(url){
    var orderBy = $("#itemsListSortBy").val().split("-")[0];
    var orderFormat = $("#itemsListSortBy").val().split("-")[1];
    var limit = $("#itemsListPerPage").val();
    var local = $("#itemsListSortByLocal").val();
    
    $.ajax({
        type:'get',
        url: url ? url : appRoot+"items/liltlocal/",
        data: {orderBy:orderBy, orderFormat:orderFormat, limit:limit, local:local},
