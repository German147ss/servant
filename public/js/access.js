...
loginForm.onsubmit = function(e){
    ...
    if(!email || !password){
        var errMsg = !email ? "Ingrese su email" : "Ingrese su contrasñea";

        document.getElementById('errMsg').innerHTML = errMsg;
    }

    else{
        document.getElementById('errMsg').innerHTML = "Verificando......";
	...
	handleLogin(email, password, function(returnedData){
	   if(returnedData.status === 1){
                document.getElementById('errMsg').innerHTML = "Verificado. Redireccionando..";
...
function handleLoginRedirect(){
    ...
    var uriToRedirectTo = currentUrl.split("red_uri=")[1] || "transactions";
