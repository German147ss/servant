...
if( $("#esAdmin").length ){
         			  currentItems[key] = currentItems[key].split("-")[0];
					}
				...
				cloned.find(".selectedItemDefault").prepend("<option value='' selected>Seleccionar Producto</option>");
    ...
    $("#transListTable").on('click', '.delItem', function(e){
        ...
        if(itemId){
            var confirm = window.confirm("Esta seguro que desea cancelar la venta?No se peude deshacer");
            ...
            if(confirm){
                ...
                $.ajax({
                    ...
                    if(rd.status === 1){
                        ...
                        changeFlashMsgContent('Venta Cancelada correctamente', '', 'green', 1000);
                    }
    ...
    $("#modeOfPayment").change(function(){
        ...
        if(modeOfPayment === "POS"){
             ...
             *
             */
            $("#amountTenderedLabel").html("Monto Pagado");
    ...
    $("#confirmSaleOrder").click(function(){
        ...
        if ( $("#idHidden").length ) {
          var local = $("#idHidden").text();
        }else if ($("#esAdmin").length) {
          var local = $("#esAdmin").val();
        }
        ...
        if(isNaN(amountTendered) || (amountTendered === '0.00') || !modeOfPayment || (amountTendered < cumAmount)){
            ...
            amountTendered < cumAmount ? $("#amountTenderedErr").html("El pago no puede ser menor "+cumAmount) : "";
            ...
            for(var i = 0; i < selectedItemNodeLength; i++){
              if($("#esAdmin").length){
                var itemDeAdmin = selectedItemNode[i].value.split("-")[0];
                var itemCode =itemDeAdmin;
                var local = selectedItemNode[i].value.split("-")[1];
              }else{
            ...
            return new Promise(function(resolve, reject){
                ...
                $.ajax({
                    ...
                    data: {_aoi:_aoi, _mop:modeOfPayment, _at:amountTendered, _cd:changeDue, _ca:cumAmount, vat:vatPercentage,
                        discount:discountPercentage, cc:custCode, cn:custName, ce:custEmail, cp:custPhone, _local:local},
	...
	var inew;
		...
		if( $("#esAdmin").length ){
			...
			console.log($(".selectedItem").length);
 			console.log($(".selectedItem").last().val());
                        //if no select input has been added or the last select input has a value (i.e. an item has been selected)
                        if(!$(".selectedItem").length || $(".selectedItem").last().val()){
                            //add a new item by triggering the clickToClone btn. This will handle everything from 'appending a list of items' to 'auto-selecting
                            //the corresponding item to the value detected by the scanner'
    ...
    $("#showTransForm").click(function(){
        ...
        if($("#newTransDiv").hasClass('collapse')){
            $(this).html("<i class='fa fa-plus'></i> Nueva Venta");
        }

        else{
            $(this).html("<i class='fa fa-minus'></i> Nueva Venta");
    ...
    $("#hideTransForm").click(function(){
        ...
        //change main "Nueva Venta" button back to default
        $("#showTransForm").html("<i class='fa fa-plus'></i> Nueva Venta");
    });
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    $("#clickToGenProducto").click(function(e){
        e.preventDefault();

        var fromDate = $("#transFromProd").val();
        var toDate = $("#transToProd").val();
        ...
        $("#transFromErr").html("");
        $("#reportModalProd").modal('hide');

        var strWindowFeatures = "width=1000,height=500,scrollbars=yes,resizable=yes";

        window.open(appRoot+"transactions/reportItems/"+fromDate+"/"+toDate, 'Print', strWindowFeatures);
        console.log(window.open(appRoot+"transactions/reportItems/"+fromDate+"/"+toDate, 'Print', strWindowFeatures))
    });
...
function calchadue(){
    ...
    if(amountTendered && (amountTendered < cumAmount)){
        $("#amountTenderedErr").html("El pago no puede ser menor $"+ cumAmount);
...
function changeSelectedItemWithBarcodeText(barcodeTextElem, selectedItem){
        if( $("#esAdmin").length ){
            var local = window.Cyberbank;
            selectedItem = selectedItem + "-" + local;
        }
